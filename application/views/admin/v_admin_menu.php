<ul id="admin_menu">
	<li><a href="/admin">Главная</a></li>
	<li><a href="/admin/category">Каталог</a>
		<ul>
		<li><a href="/admin/products">Товары</a></li>
		<li><a href="/admin/manufactures">Производительи</a></li>
		</ul>
	</li>
	<li><a href="/admin/orders">Заказы</a></li>
	<li><a href="/admin/pages">Страницы</a></li>
    <li><a href="/admin/articles">Статии</a></li>
	<li>
	<a href="/admin/modules">Модульи</a>
		<ul>
			<li><a href="/admin/modules/delivery">Доставка</a></li>
			<li><a href="/admin/modules/payments">Оплата</a></li>
		</ul>
	</li>
	
	<li><a href="/admin/banners">Ваннеры</a></li>
	<li><a href="/admin/system">Система</a>
		<ul>
			<li><a href="/admin/users">Пользовательи</a></li>
			<li><a href="/admin/system/settings">Настройки</a></li>
			<li>
			<a href="#">Локализация</a>
			<ul>
			<li><a href="/admin/countries">Страны и Регионы</a></li>
			<li><a href="#">Валюта</a></li>
			</ul>
			</li>
		</ul>
	</li>
</ul>
